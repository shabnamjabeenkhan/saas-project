...
Learn how to debug any issue systematically using AI assistance and the right tools.

<Note>
**Context is key!** The more context you provide, the faster you'll solve the bug.
</Note>
...
✅ **Terminal logs** - Full error stack traces  
✅ **Browser console** - JavaScript errors  
✅ **Network tab** - Failed requests  
✅ **Code snippets** - Relevant file sections  
✅ **Environment** - Dev vs production, OS, browser  
...
Network tab shows:
- POST /api/subscribe → 500 Internal Server Error
...
// Use browser DevTools → Elements → Computed → z-index
```
...
// Verify in Convex Dashboard → Settings → Clerk Integration
...
2. Resend Dashboard  
3. `.env.local` → `FRONTEND_URL`
4. `vite.config.ts` → `allowedHosts`
...
3. Check Polar.sh Dashboard → Webhooks → Delivery logs
...
See [Observability](/building-with-kaizen/observability) for full Sentry setup.
...
<Cards>
  ...
  <Card title="Performance" href="/building-with-kaizen/performance" description="Optimize your app" />
