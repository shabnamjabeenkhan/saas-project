...
- ✅ Application working locally with all features configured
- ✅ All API keys and environment variables gathered
...
From Polar Dashboard → **Settings** → **General**:
- **Access Token**: Settings → General → Developers → New Token (all scopes, no expiration)
- **Organization ID**: Settings → General → Profile → Identifier
- **Server**: Set to `production`

#### Configure Webhook
1. Get your Convex production HTTP URL (ends in `.convex.site`)
2. In Polar Dashboard → **Settings** → **Webhooks**
...
- ✅ Visit `https://www.yourdomain.com` - should redirect or load
...
- ✅ Social logins work (if configured)
...
:::note
  🎉 Congratulations! Your Kaizen application and documentation are now live in production. Start building amazing features for your users!
:::
